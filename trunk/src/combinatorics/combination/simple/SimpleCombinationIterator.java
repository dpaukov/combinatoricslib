 ...
 * 
 * @param <T> Type of elements in a combination
 */
public class SimpleCombinationIterator<T> implements Iterator<CombinatoricsVector<T>> {
   ...
   * Constructor
   * 
   * @param generator Generator of simple combinations
   */
  ...
  public void next() {
    ...
    for (int i = 1; i <= _lengthK; i++) {
      ...
      if (_generator.getCoreObject().getSize() > 0) {
        _currentSimpleCombination.setValue(i - 1, _generator.getCoreObject().getValue(index));
      }
    }
