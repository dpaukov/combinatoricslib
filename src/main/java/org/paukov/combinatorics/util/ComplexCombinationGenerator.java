 ...
 *
 * It is possible to generate non-overlapping sublists of length <b>n</b> of a given <b>list</b> For
 * example, if a list is (A, B, B, C), then the non-overlapping sublists of length 2 will be:
 * <pre>
 * ( (A), (B, B, C) )
 * ( (B, B, C), (A) )
 * ( (B), (A, B, C) )
 ...
 * ICombinatoricsVector&lt;String&gt; vector = Factory.createVector(new String[] { "A", "B", "B",
 * "C" });
 *
 * // Create a complex-combination generator
 * Generator&lt;ICombinatoricsVector&lt;String&gt;&gt; gen = new ComplexCombinationGenerator&lt;String&gt;(vector,
 * 2);
 *
 ...
 *
 * <pre>
 * ([A],[B, B, C]) - CombinatoricsVector=([CombinatoricsVector=([A], size=1),
 * CombinatoricsVector=([B, B, C], size=3)], size=2)
 * ([B, B, C],[A]) - CombinatoricsVector=([CombinatoricsVector=([B, B, C], size=3),
 * CombinatoricsVector=([A], size=1)], size=2)
 * ([B],[A, B, C]) - CombinatoricsVector=([CombinatoricsVector=([B], size=1),
 * CombinatoricsVector=([A, B, C], size=3)], size=2)
 * ([A, B, C],[B]) - CombinatoricsVector=([CombinatoricsVector=([A, B, C], size=3),
 * CombinatoricsVector=([B], size=1)], size=2)
 * ([A, B],[B, C]) - CombinatoricsVector=([CombinatoricsVector=([A, B], size=2),
 * CombinatoricsVector=([B, C], size=2)], size=2)
 * ([B, C],[A, B]) - CombinatoricsVector=([CombinatoricsVector=([B, C], size=2),
 * CombinatoricsVector=([A, B], size=2)], size=2)
 * ([B, B],[A, C]) - CombinatoricsVector=([CombinatoricsVector=([B, B], size=2),
 * CombinatoricsVector=([A, C], size=2)], size=2)
 * ([A, C],[B, B]) - CombinatoricsVector=([CombinatoricsVector=([A, C], size=2),
 * CombinatoricsVector=([B, B], size=2)], size=2)
 * ([A, B, B],[C]) - CombinatoricsVector=([CombinatoricsVector=([A, B, B], size=3),
 * CombinatoricsVector=([C], size=1)], size=2)
 * ([C],[A, B, B]) - CombinatoricsVector=([CombinatoricsVector=([C], size=1),
 * CombinatoricsVector=([A, B, B], size=3)], size=2)
 * </pre>
 *
...
public class ComplexCombinationGenerator<T> extends
   ...
   * Constructor
   *
   * @param originalVector Original vector/set which is used for combination generation
   ...
   * @param originalVector Original vector/set which is used for combination generation
   * @param combinationsLength Length of combination to generate
   * @param isOrderImportant true if the order of the generated combinations is important
   */
  public ComplexCombinationGenerator(ICombinatoricsVector<T> originalVector,
   ...
   * @param combinationsLength Length of combination to generate
   * @param isOrderImportant true if the order of the generated combinations is important
   * @param excludeEmptySet true if the empty set has to be excluded from the generated
   * combinations
   */
  public ComplexCombinationGenerator(ICombinatoricsVector<T> originalVector,
      int combinationsLength, boolean isOrderImportant,
  ...
  public ICombinatoricsVector<ICombinatoricsVector<T>> getOriginalVector() {
    ICombinatoricsVector<ICombinatoricsVector<T>> result = createVector();
